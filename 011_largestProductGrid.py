#!/usr/bin/python
'''Project Euler - Problem 11 (Leonardo Dias - 11/05/2020)
...
def prodHor(grid):
    maxProd = 0
    for row in grid:
        for inicio in range(len(row)-3):
            prod = 1
            for quadra in range(inicio, inicio+4):
                prod *= int(row[quadra])
            if prod>maxProd: maxProd = prod
    return maxProd
...
def prodVert(grid):
    ...
    for row in grid:
        ...
        for index in range(len(row)):
            column[index].append(int(row[index]))
    for posicao in range(len(column)-3):
        prod=1
        for quadra in range(posicao, posicao+4):
            prod *= int(column[posicao][quadra])
        if prod>maxProd: maxProd = prod;
    return maxProd

if __name__=='__main__':
    # import re; import numpy as np; GRID = re.findall(r'(\d\d)\s?', -coloquei a lista aqui-); np.array(GRID).resize(20, 20)
    GRID= [['08', '02', '22', '97', '38', '15', '00', '40', '00', '75', '04',
    ...
    print(max(prodDiagRight(GRID), prodDiagLeft(GRID), prodVert(GRID), prodHor(GRID)))
